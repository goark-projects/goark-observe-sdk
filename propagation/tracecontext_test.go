...
func TestTraceContextRoundTrip(t *testing.T) {
	...
	ctx := observe.ContextWithSpanContext(context.Background(), observe.NewSpanContext(traceID, spanID, observe.TraceFlagsSampled, "vendor=value", false))
	...
	if extracted.TraceState != "vendor=value" {
		t.Fatalf("tracestate = %q", extracted.TraceState)
	}
}
